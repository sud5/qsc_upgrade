...
//        return $this->output->header();
        ...
        // Below heaing variable Default heading commented as per client needs

        //$heading = format_string($header->assign->name, false, array('context' => $header->context));

        //Customization Sameer for course title Start
        $heading = format_string($this->page->course->fullname);
        $o .= $this->output->heading($heading);
        //Customization Sameer for course title Ends
        if(optional_param('action','',PARAM_RAW) == "editsubmission"){
             if ($header->preface) {
                 $o .= $header->preface;
             }
             $o .= $this->output->box_start('generalbox boxaligncenter', 'intro');
             $o .= "<h2 style='font-size:22px;'>".$header->assign->sub."</h2>";
             $o .= "<div>".$header->assign->subeditor."</div>";
             $o .= $header->postfix;
             $o .= $this->output->box_end();

        }else{
            //Customization Sameer for submission description Ends
            ...
            if ($header->showintro) {
                ...
                $o .= $this->output->box_end();
                //$myarray = $this->page;
                // For certification section display start
                if(!is_siteadmin($USER)){
                    $cmCertificateData = $DB->get_record('course_modules', array('course'=>$this->page->course->id,'module'=>25), "*");
                    $o .= $this->output->box_start('generalbox cg_box', 'assign_passed_msg');
                    
                    $chk_course_completions = $DB->get_record('course_completions', array('userid'=>$USER->id, 'course' => $this->page->course->id), "id, timecompleted, reaggregate");
                    $chk_old_user = $DB->get_record('user', array('id'=>$USER->id), "id, qsc_training_id");
                    if($chk_course_completions->reaggregate != '' && $chk_course_completions->reaggregate != 0 && $chk_old_user->qsc_training_id == NULL){ 
                        $dlinkstatus = html_writer::tag('span',get_string('reaggregatedownloadlink','mod_assign'),array('class'=>'download-certificate-link'));
                        /* ======== New modules added and Renewal Certification Process */

                    }elseif($chk_course_completions->timecompleted != '' && $chk_course_completions->timecompleted != 0){
                       /* ======== New modules added and Renewal Certification Process - START BY PAWAN/modify by shiva ===================  */
                        $dlinkstatus = "<a href='/mod/simplecertificate/view.php?id=".$cmCertificateData->id."&action=get'>".get_string('downloadlink2','mod_assign')."</a>";
                       $dlinkstatus  = html_writer::link(new \moodle_url('/mod/simplecertificate/view.php',['id'=>$cmCertificateData->id,'action'=>'get']),get_string('downloadlink2','mod_assign'));
                        /* ======== New modules added and Renewal Certification Process - END BY PAWAN ===================  */
                    }
                    $o .= str_replace("{DOWNLOAD_LINK}", $dlinkstatus, $header->assign->downloaddetails);
                    $o .= $this->output->box_end();
                }  // For certification section display end
            }
        }
...
//        $o .= $this->output->heading(get_string('feedback', 'assign'), 3);
    ...
    public function render_assign_submission_status_compact(\assign_submission_status_compact $status) {
        ...
        if ($submission) {
//Customize 11678 US if grade status is not in passed mode.
            $_SESSION['gs']=$status->gradingstatus;
            if ((!$status->teamsubmission || $status->submissiongroup != false || !$status->preventsubmissionnotingroup) && $status->gradingstatus != ASSIGN_GRADING_STATUS_GRADED) {
...
//    }
        ...
        if(!is_siteadmin($USER)){
            ...
            $o .= $this->output->heading(get_string('submissionstatusheading', 'assign'), 3, 'submissionheading');
            $time = time();
            ...
            if ($status->attemptreopenmethod != ASSIGN_ATTEMPT_REOPEN_METHOD_NONE) {
                ...
                $row = new \html_table_row();
                //$row->attributes['class'] = 'submission_remove';

                $cell1 = new \html_table_cell(get_string('attemptnumber', 'assign'));
                $maxattempts = $status->maxattempts;
                if ($maxattempts == ASSIGN_UNLIMITED_ATTEMPTS) {

                    if($currentattempt >= 2 && $status->submission->status != 'submitted'){
            ...
            if (!$status->teamsubmissionenabled) { 
                if ($status->submission && $status->submission->status != ASSIGN_SUBMISSION_STATUS_NEW) {
                    if($status->submission->status == 'reopened'){

                        // $statusstr = 'submissionstatus_reopened';
                        //$cell2 = new \html_table_cell(get_string($statusstr."assign"));
                        //$cell2->attributes = array('class'=>'submissionstatusgray');
                    }else{
                        $row = new \html_table_row();
            ...
            if ($status->locked) {
                ...
                //Submission status should be removed as wediscussed in 3rd Aug meeting
                //noattempt=Awaiting Student Submission (no color) (Submission)
                //Submitted for grading  (yellow) (Submission)
                //Grading Status
                //notgraded=Not graded  (No color on the bar) (In moodle lms case this is Grading status)
                //Reopened- Revisions requested by Instructor   (Bar should be red with white text) (In moodle lms case this is Submission status)
                //graded- Passing Score!  (Bar should be green) (In moodle lms case this is Grading status)
                // Grading status.
                $row = new \html_table_row();
            ...
            $flagGreen = 1;
                $cell2->attributes = array('class' => 'grading-green');
            } else {
                $cell2->attributes = array('class' => 'submissionnotgraded');
                //$cell2->attributes = array('class' => 'grading-red');
            }
            $row->cells = array($cell1, $cell2);
            $t->data[] = $row;
            ...
            // Last modified and Feedback. //Prod change 1
            if ($submission) {
                // Feedback from plugins.
                $cmfObj = $DB->get_record_sql('SELECT instance FROM mdl_course_modules WHERE id = ?' , array($status->coursemoduleid, $USER->id));
                $gradeseObj = $DB->get_record_sql('SELECT id FROM mdl_assign_grades WHERE assignment = ? AND userid = ? AND attemptnumber=? order by attemptnumber desc limit 0,1', array($cmfObj->instance, $USER->id,$status->submission->attemptnumber));
                    if($status->gradingstatus == 'graded'){
                        $afc_params = array('grade'=>$gradeseObj->id);
                        $afcObj = $DB->get_record('assignfeedback_comments', $afc_params, '*');
                        //Submission Feedback code start
                        if($flagGreen == 1){
                            $cell1 = new \html_table_cell("Feedback");
                            $cell2 = new \html_table_cell($afcObj->commenttext);
...
//Customize 11678 US if grade status is not in passed mode.
 ...
 unset($_SESSION['gs']);
            ...
            //Customisation for assumption to swap one row(Option Submission Message) to another row (File Submission)
                    ...
                    if (!$submission || $submission->status == ASSIGN_SUBMISSION_STATUS_NEW) {
                        if($flagGreen != 1){
                                    ...
                                    $o .= $this->output->single_button(new \moodle_url('/mod/assign/view.php', $urlparams),
                                                                       get_string('continuesubmission', 'assign'), 'get');
                                    $o .= $this->output->box_end();
                        }
                       ...
                       /* $o .= $this->output->box_start('generalbox submissionaction');
                        ...
                        $o .= $this->output->box_start('boxaligncenter submithelp');*/
                         //Customisation for remove text Make changes to your submission
...
//    }
    ...
    public function render_assign_attempt_history(\assign_attempt_history $history) {
        global $USER,$DB,$PAGE,$CFG,$COURSE,$SESSION; 
        //GradePanel - start
//        $sql = "SELECT id FROM {role} WHERE shortname='grader'";
//        $roleobj = $DB->get_record_sql($sql);
        $roleobj = $DB->get_record('role', ['shortname'=>'grader']);
        //$rolesql = "SELECT contextid FROM {role_assignments} where userid=".$USER->id." AND roleid=".$roleobj->id;
        //GradePanel - end
    
        if(!is_siteadmin($USER) && !$DB->record_exists('role_assignments',['userid'=>$USER->id,'roleid'=>$roleobj->id])){
            $o = ''; 
            ...
            foreach ($history->submissions as $i => $submission) {
                ...
                if ($submission) {
                    //Customisation start for Grading status with reopened status
                    $cell3 = new \html_table_cell(get_string('gradingstatus', 'assign'));
                    //$cell2 = new \html_table_cell(get_string('submissionstatus_' . $submission->status, 'assign'));
                    //Customisation Sameer as per requirement
                    if($submission->status == "submitted" || $submission->status == "marked"){
                        $cell4 = new \html_table_cell(get_string('submissionstatus_reopened', 'assign'));
                        $cell4->attributes = array('class'=>'submissionstatusreopened');
                    }
                    else{
                        $cell4 = new \html_table_cell(get_string('submissionstatus_' . $submission->status, 'assign'));
                        $cell4->attributes = array('class'=>'submissionstatusreopened'); //pre added
                    }
                    //Customisation end
                    $t->data[] = new \html_table_row(array($cell3, $cell4));
                    //US 11678
                    if( $_SESSION['gs'] != ASSIGN_GRADING_STATUS_GRADED){
                         unset($_SESSION['gs']);
                        foreach ($history->submissionplugins as $plugin) {
                            $pluginshowsummary = !$plugin->is_empty($submission) || !$plugin->allow_submissions();
                            if ($plugin->is_enabled() &&
            ...
            $containerid = 'attempthistory' . uniqid();
            $o .= $this->heading(get_string('comment','assign'));
            $o .= $this->box_start('attempthistory', $containerid);
            // --  -- --- - -Start - Exam Comment - View admin comment detailed page comment thread  - - - - - - //
            $status = $history->submissionplugins;
            $submission=$history->submission;
            if(!empty($status) && !empty($submission)){
                $sqlAssignSubmission2 = "SELECT id,status,timemodified FROM {assign_submission} WHERE id=$submission->id";
                $rsAssignSubmission2 = $DB->get_record_sql($sqlAssignSubmission2);
                if($rsAssignSubmission2->status == 'reopened'){
                    $timemodified_notsubmit = $rsAssignSubmission2->timemodified;
                    if ($timemodified_notsubmit) {
                        $submissionsummary2 = userdate($timemodified_notsubmit);
                    } else {
                        $submissionsummary2 = get_string('nosubmission', 'assign');
                    }
                    $attemptnotsubmitparams = array('attemptnumber'=>$submission->attemptnumber+1,
                                              'submissionsummary'=>$submissionsummary2);
                    $o .= $this->heading(get_string('attemptheading', 'assign', $attemptnotsubmitparams), 4);
                    $t = new \html_table();

                    foreach ($status as $plugin) {        
                        $pluginshowsummary = !$plugin->is_empty($submission) || !$plugin->allow_submissions();
                        if ($plugin->is_enabled() && $plugin->is_visible() && $plugin->has_user_summary() && $pluginshowsummary){
                            //echo "in foreach <pre>"; print_r($plugin); exit;
                            $row = new \html_table_row();
                            $cell1 = new \html_table_cell($plugin->get_name());
                            $displaymode = assign_submission_plugin_submission::SUMMARY;
                            $pluginsubmission = new assign_submission_plugin_submission($plugin,
           ...
           // --  -- --- - -End -Exam Comment - - - - - - //
            foreach ($history->submissions as $i => $submission) {
                //Grader Dashboard bug fixing
                $flagC=0;
                if($i==0){
                    //echo $i;
                    $sqlAssignSubmission = "SELECT id,status FROM {assign_submission} WHERE id=$submission->id";
                    $rsAssignSubmission = $DB->get_record_sql($sqlAssignSubmission);
                    if($rsAssignSubmission->status == 'reopened'){
                     $flagC=1;
                    }else{
                     $flagC=0;
                    }
                ...
                if($flagC==1){
                    if($i >= 1 || count($history->submissions) == 1){ 
                        $grade = null; 
                        foreach ($history->grades as $onegrade) {
                            if ($onegrade->attemptnumber == $submission->attemptnumber) {
                                $grade = $onegrade;
    ...
    public function render_assign_feedback_plugin_feedback(\assign_feedback_plugin_feedback $feedbackplugin) {
        ...
        if ($feedbackplugin->view == \assign_feedback_plugin_feedback::SUMMARY) {
            ...
            //$o .= $this->output->box_start('boxaligncenter plugincontentsummary summary_' . $classsuffix);
            //FC 1st point code start
                       // $o .= $this->output->box_start('boxaligncenter plugincontentsummary summary_' . $classsuffix);
            // if(strlen($summary) > 140){
            //                 //pre
            //                 //$o .= $this->output->box_start('boxaligncenter hidefull full_' . $classsuffix);
            //                 $o .= $this->output->box_start('boxaligncenter full_' . $classsuffix);
            // }else{
            //  $o .= $this->output->box_start('boxaligncenter summary_' . $classsuffix);
            // }
            $o .= $this->output->box_start('boxaligncenter full_' . $classsuffix);
            //FC 1st point code end
...
//                $o .= \html_writer::link('', $expandicon, $options);
                //FC 1st point code start
                $options = array('class'=>'expandsummaryicon contract_' . $classsuffix);
                //FC 1st point code end
