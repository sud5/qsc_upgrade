...
class mod_facetoface_session_form extends moodleform {
    ...
    public function definition() {
        ...
        $context = context_course::instance($this->_customdata['course']->id);
        facetoface_add_customfields_to_form_new($mform, $customfields, false, $context->id);
        ...
        if ($mform->elementExists('custom_managersemail')) {
            $mform->addHelpButton('custom_managersemail', 'managersemail', 'facetoface');
        }
        //US 3072 start
        $mform->addElement('checkbox', 'countryrestriction', get_string('countryrestriction', 'facetoface'));
        //US 3072 end
...
//        $repeatarray[] = &$mform->createElement('html', html_writer::empty_tag('br')); // Spacer.
        ...
        $mform->addElement('editor', 'details_editor', get_string('logos', 'facetoface'), null, $editoroptions);
