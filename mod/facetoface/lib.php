...
define('CUSTOMFIELD_TYPE_TEXTAREA',    3);
...
define('MDL_F2F_STATUS_ATTENDED',           110);
...
function facetoface_statuses() {
    ...
    return array(
        ...
        MDL_F2F_STATUS_ATTENDED            => 'attended',
    );
}
...
function facetoface_fix_settings($facetoface) {
    ...
    //US #6050 start
     if (empty($facetoface->emailmanagerlearnerprerequisite)) {
        $facetoface->prerequisiteinstrmngr = null;
    }
    //US #6050 end
...
function facetoface_add_instance($facetoface) {
    ...
    if ($sessions = facetoface_get_sessions($facetoface->id, null, null, null)) {
...
function facetoface_update_attendees($session) {
    ...
    // Update user status'.
    // Task #20393 start
    $users = facetoface_get_attendees($session->id, 'display_order');
    // Task #20393 end
    if ($users) {
            ...
            if ($booked < $capacity) {
                foreach ($users as $user) {
                    ...
                    if ($user->statuscode == MDL_F2F_STATUS_WAITLISTED) {
                        ...
                        if (!facetoface_user_signup($session, $facetoface, $course, $user->discountcode, $user->notificationtype, MDL_F2F_STATUS_BOOKED, $user->id,true,null,"conversion")) {
                            return false;
                        }
                        $booked++;
                    }
    ...
    if ($signedupusers and count($signedupusers) > 0) {
        foreach ($signedupusers as $user) {
            if (facetoface_user_cancel($session, $user->userid, true)) {
                if ($isadmin) {
                    facetoface_send_cancellation_notice($facetoface, $session, $user->userid, true);
                } else {
...
//63 Est Start
function email_mgr_field_addition($userid, $cat_id, $course_id) {
    global $CFG, $DB, $OUTPUT;

    $coursesData = $DB->get_records('course_completion_criteria', array('course' => $course_id, 'criteriatype' => 8), '', 'id, course, courseinstance');
    ...
    if (count($coursesData) == 1) {
        ...
        if ($pri_course_category) {
            ...
            $sql = "SELECT id as courseinstance FROM {course} c WHERE id = $course_id and visible=1 order by id";
            // Addition Certification Columns end
            $rs = $DB->get_records_sql($sql);
        }
    }
    ...
    if ($course_id == 86) {
        $sql = "SELECT id as courseinstance FROM {course} c WHERE id=49";
        // Addition Certification Columns end
        $rs = $DB->get_records_sql($sql);
    }
    if (empty($rs)) {
        $sql = "SELECT id as courseinstance FROM {course} c WHERE id=7";
            ...
            if ($flagrs == 0) {
                ...
                if ($silObjData) {
                    ...
                    if (empty($scilObjData)) {
                        ...
                        //60 Est end
                        //Level One Column on Admin Pages Start
                        if ($scilObjData->timeexpired < time()) {
                ...
                if ($flagrs == 0 || $flagrs == 3) {
                    ...
                    if (!empty($rsStudentRecordArray)) {
                        $flagrs = 2;
                        $statusCourseComp = get_string('submissionstatus_' . $rsStudentRecordArray->status, 'assign');
                    } else {
                        //60 est start
                        if ($cidforsubmitgradingflag == 0 || $cidforsubmitgradingflag == '' && empty($silObjData)) {
                            $statusCourseComp = "NOT CERTIFIED";
                            $flagrs = 0;
                        }
                        //60 est end
    ...
    if ($flagrs == 0) {
        ...
        $sqlrepeat = "SELECT id as courseinstance FROM {course} c WHERE category = $cat_id order by id";
        $rsrepeat = $DB->get_records_sql($sqlrepeat);

        //Bug #8543 fixes start for Level2 cinema type
        if ($course_id == 86) {
            $rsrepeat = $rs;
        }
        //Bug #8543 fixes end for Level2 cinema type
        ...
        if ($rsrepeat) {
            ...
            foreach ($rsrepeat as $rsrepeatkey) {
                ...
                if ($sectionDataRs) {
                    $cmIdsArr = array();
                    $sectionSeqCnt = $moduleCompleted = 0;
            ...
            if ($flagStat != 0) {
                ...
                $statusCourseComp = '<span style="color:red">' . max($moduleCompletedArr['cntCMC']) . ' out of ' . $moduleCompletedArr['cntTotCM'][$cntTotCMKeyInitial] . ' modules completed ' . '</span>';
                if ($statusCourseComp == '') {
 ...
 * @return  string
 */
function facetoface_email_substitutions($msg, $facetofacename, $reminderperiod, $user, $data, $sessionid, $trpFlag=0) {
    global $CFG, $DB;
    //trp start
    if ($trpFlag == 0) {
        $fdata = $DB->get_record('facetoface_signups', array('sessionid' => $sessionid, 'userid' => $user->id), 'id,diet_restriction,who_refer_you_desc,who_refer_you_title,zipcode,country,comment');
    } else {
        $fdata = $_SESSION['trpPostValues'];
        unset($_SESSION['trpPostValues']);
    }
    //trp end
    if (empty($msg)) {
        return '';
    ...
    if ($data->datetimeknown) {
        ...
        // Scheduled session.
        //Bug #3467 start
        if ($data->sessiondates[0]->timestart == "") {
            unset($data->sessiondates);
            $data->sessiondates = facetoface_get_session_dates($sessionid);
        }
        //Bug #3467 end

        $datestartchk = date('dmy', $data->sessiondates[0]->timestart);
        $dateendchk = date('dmy', $data->sessiondates[0]->timefinish);

        /* Customization for date formate start by - pawan 27Nov */
        $timestartStr = $data->sessiondates[0]->timestart;
        $timefinishStr = $data->sessiondates[0]->timefinish;
        ...
        if ($dateend != '') {
            $sessiondate = $datestart . ' - ' . $dateend;
        } else {
            $sessiondate = $datestart;
        }


        $starttime = date('h:i A', $data->sessiondates[0]->timestart);
        $finishtime = date('h:i A', $data->sessiondates[0]->timefinish);
        ...
        foreach ($data->sessiondates as $date) {
            ...
            if ($data->sessiondates[0]->timestart != "2481889900" && $data->sessiondates[0]->timefinish != "2481889900") {
...
//    $msg = str_replace(get_string('placeholder:cost', 'facetoface'), facetoface_cost($user->id, $sessionid, $data, false), $msg);
    $msg = str_replace(get_string('placeholder:Company', 'facetoface'), $user->institution, $msg);
    $msg = str_replace(get_string('placeholder:EmailAddress', 'facetoface'), $user->username, $msg);
    $msg = str_replace(get_string('placeholder:PhoneNumber', 'facetoface'), $user->phone1, $msg);

    //63 Est Start
    if(!empty($sessionid)) {
        $getCourseData = $DB->get_record_sql("SELECT face.course FROM `mdl_facetoface_sessions` AS fs RIGHT JOIN `mdl_facetoface` AS face ON face.id = fs.facetoface WHERE fs.id = $sessionid");
        $course_id_pre = $getCourseData->course;
    }
     if( $course_id_pre == 42 ){
        $cat_id = 4;
     }
     ...
     if($course_id_pre){
    $lOneCertificateStatus = email_mgr_field_addition($user->id, $cat_id, $course_id_pre);

    $msg = str_replace(get_string('placeholder:LevelOneCertified', 'facetoface'), $lOneCertificateStatus, $msg);
}
    //63 Est End

    $msg = str_replace(get_string('placeholder:Address', 'facetoface'), $user->address, $msg);
    $msg = str_replace(get_string('placeholder:City', 'facetoface'), $user->city, $msg);
    $msg = str_replace(get_string('placeholder:State', 'facetoface'), $user->state, $msg);
    $msg = str_replace(get_string('placeholder:Zipcode', 'facetoface'), $user->zip, $msg);
    $msg = str_replace(get_string('placeholder:Country', 'facetoface'), @$countries[$user->country], $msg);
    if(!empty($sessionid)) {
        $msg = str_replace(get_string('placeholder:cost', 'facetoface'), facetoface_cost($user->id, $sessionid, $data, false), $msg);
    }
if($data->sessiondates[0]->timestart != "2481889900" && $data->sessiondates[0]->timefinish != "2481889900")
{
    ...
    $msg = str_replace(get_string('placeholder:Date', 'facetoface'), $sessiondate, $msg);
    ...
    $msg = str_replace(get_string('placeholder:Time', 'facetoface'), $starttime ." - ". $finishtime, $msg);
    $msg = str_replace("Cheers,<br>", 'Cheers,', $msg);
    }else{
    $msg = str_replace(get_string('placeholder:alldates', 'facetoface'), '', $msg);
    $msg = str_replace(get_string('placeholder:sessiondate', 'facetoface'), '', $msg);
    $msg = str_replace(get_string('placeholder:Date', 'facetoface'), '', $msg);
    $msg = str_replace(get_string('placeholder:starttime', 'facetoface'), '', $msg);
    $msg = str_replace(get_string('placeholder:finishtime', 'facetoface'), '', $msg);
//TRP start
    //$msg = str_replace(get_string('placeholder:Time', 'facetoface'), '', $msg);
    $msg = str_replace(get_string('placeholder:Time', 'facetoface'). " <br>", '', $msg);
    $msg = str_replace("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; [session:Location]","[session:Location]",$msg);
//TRP end
    /* Customization for remove date and time lebel when date and time no define start section - Pawan 01-Dec - 2017 */
    // add comma after user firstname - start
    $msg = str_replace("Date:", ' ', $msg);
    // add comma after user firstname - end
//TRP start
    // add comma after user firstname - start
    $msg = str_replace(" ,", '', $msg);
    // add comma after user firstname - end
...
//TRP end
    ...
    /* Customization for remove date and time lebel when date and time no define end section - Pawan 01-Dec - 2017 */
}
    ...
    if ($fdata->who_refer_you_desc != '') {
        $msg = str_replace(get_string('placeholder:diet', 'facetoface'), "<br> Dietary Information: " . $fdata->who_refer_you_desc . "<br><br>", $msg);
    } else {
        //$msg = str_replace(get_string('placeholder:diet', 'facetoface'), '<br><br>', $msg);
        /* Customization for remove extra space in bottom start section - Pawan 01-Dec - 2017 */
        $msg = str_replace(get_string('placeholder:diet', 'facetoface'), '', $msg);
        /* Customization for remove extra space in bottom end section - Pawan 01-Dec - 2017 */
    }

    if ($fdata->who_refer_you_title != '') {
        $msg = str_replace(get_string('placeholder:rtitle', 'facetoface'), "<br> Recommend: " . $fdata->who_refer_you_title, $msg);
    } else {
        $msg = str_replace(get_string('placeholder:rtitle', 'facetoface'), '<br><br>', $msg);
    }


    /* SUBMIT A LOCAL TRAINING REQUEST (Point -3) BY PAWAN - START HERE */

    if ($fdata->country != '') {
        $msg = str_replace(get_string('placeholder:country_request', 'facetoface'), "Country: " . $fdata->country, $msg);
    } else {
        $msg = str_replace(get_string('placeholder:country_request', 'facetoface'), "", $msg);
    }

    if ($fdata->zipcode != ' ') {
        $msg = str_replace(get_string('placeholder:zipcode_request', 'facetoface'), "Zipcode: " . $fdata->zipcode, $msg);
    } else {
        $msg = str_replace(get_string('placeholder:zipcode_request', 'facetoface'), "", $msg);
    }

    if ($fdata->comment != '') {
        $msg = str_replace(get_string('placeholder:comment_request', 'facetoface'), "Comment: " . $fdata->comment, $msg);
    } else {
        $msg = str_replace(get_string('placeholder:comment_request', 'facetoface'), "", $msg);
    }
    /* SUBMIT A LOCAL TRAINING REQUEST (Point -3) BY PAWAN - END HERE */
    ...
    if (!empty($sessionid)) {
        ...
        $CourseData = $DB->get_record_sql("SELECT face.course FROM `mdl_facetoface_sessions` AS fs RIGHT JOIN `mdl_facetoface` AS face ON face.id = fs.facetoface WHERE fs.id = $sessionid");
        if (!empty($CourseData)) {
            $context = context_course::instance($CourseData->course);
    ...
    foreach ($customfields as $field) {
        ...
        if (!empty($customdata[$field->id])) {
            if (CUSTOMFIELD_TYPE_MULTISELECT == $field->type) {
                ...
                if (!empty($contextId) && ($field->shortname == 'qscinstructorname' || $field->shortname == 'repinstructorname')) {
                    $selectedArr = str_replace(CUSTOMFIELD_DELIMITER, "','", $customdata[$field->id]->data);
                    if (($field->shortname == 'qscinstructorname') && ($customdata[5]->data == "Inside QSC" || $customdata[5]->data == "Outside QSC")) {
                        $QSC_enrollInst = $DB->get_record_sql("SELECT GROUP_CONCAT(possiblevalues) as qscselectedvalues FROM {enrolled_instructor} WHERE possiblevalues IN ('" . $selectedArr . "')  AND  shortname ='qscinstructorname' AND contextid= $contextId");
                        if (!empty($QSC_enrollInst)) {
                            if (!empty($QSC_enrollInst->qscselectedvalues)) {
                                $value = $QSC_enrollInst->qscselectedvalues;
                            }
                        }
                    } elseif (($field->shortname == "repinstructorname") && ($customdata[5]->data == "Local Classes")) {
                        $Rep_enrollInst = $DB->get_record_sql("SELECT GROUP_CONCAT(possiblevalues) as repselectedvalues FROM {enrolled_instructor} WHERE possiblevalues IN ('" . $selectedArr . "')  AND  shortname ='repinstructorname' AND contextid= $contextId");
                        if (!empty($Rep_enrollInst)) {
                            if (!empty($Rep_enrollInst->repselectedvalues)) {
                                $value = $Rep_enrollInst->repselectedvalues;
                            }
                        }
                    }
                } else {
                    // - --  --End -New Feature Request: Editable Instructor Name Field- - - ----- ----//
 ...
 */
//US 5635: Parameter update for userrole
function facetoface_get_sessions($facetofaceid, $location='',$courseid=null, $userrole, $userRoles = []) {
    global $CFG, $DB, $USER;

    $crole = optional_param('crole', '', PARAM_TEXT); // Course Module ID.

    // Addition code
    $hasregionaladminrole = 0;
    if($USER->usertype == 'regionaladmin'){
        $hasregionaladminrole = 1;
    } else if(in_array('regionaladmin', $userRoles) && $crole != ''){
        $hasregionaladminrole = 1;
    }

    $selectrows = array();
    if ($hasregionaladminrole) {
        if (($USER->country == 'US' || $USER->country == 'USA') && $USER->zip != '') {
            $selectrows = $DB->get_records_sql("SELECT ocrs.* FROM {zip_region_subregion_import_new} zrin RIGHT JOIN {other_countries_region_subregion} ocrs ON zrin.region = ocrs.region WHERE zrin.zip = ?", array($USER->zip));
            if (empty($selectrows)){
                //$selectrows = array('US','USA','United States');
            }
        } elseif ($USER->country != 'US' || $USER->country != 'USA'){
            $countryrow = $DB->get_record_sql("SELECT c.nicename FROM country c WHERE c.iso3 = ? OR c.iso = ?", array($USER->country,$USER->country));

            $selectrow = $DB->get_record_sql("SELECT ocrs.region FROM {other_countries_region_subregion} ocrs WHERE ocrs.country = ?", array($countryrow->nicename));
            if($selectrow){
                $selectrows = $DB->get_records_sql("SELECT ocrs.country FROM {other_countries_region_subregion} ocrs WHERE ocrs.region = ?", array($selectrow->region));
            }
        }
    }
    // Addition code
    ...
    if (($USER->country == 'US' || $USER->country == 'USA') && $hasregionaladminrole) {
        $countryArray[] = 'US';
        $countryArray[] = 'USA';
    ...
    if(!empty($countryArray)){
        ...
        $countries = '"' . $countries . '"'; 
        $locationwhere .= " AND d.data IN (" . $countries . ") AND d.fieldid = 14";    
    }
    // Addition code

    //US #5635 : Task 5842 add condition for session creator role
    if($userrole == 'sessioncreator') {
        $locationwhere .= "AND s.createdby = ?";
        $locationparams[] = $USER->id;
    ...
    if ($sessions) {
        foreach ($sessions as $key => $value) {
            ...
            if (!empty($sessions[$key]->details)) {
            $cm = get_coursemodule_from_instance('facetoface', $facetofaceid, $courseid);
                ...
                $details->id = isset($sessions) ? $sessions[$key]->id : 0;
                $details->details = isset($sessions[$key]->details) ? $sessions[$key]->details : '';
                $details->detailsformat = FORMAT_HTML;
                $isadmin = is_siteadmin($USER);
                if(isguestuser() && $_SERVER['SCRIPT_NAME'] =='/course/view.php'){
                    $details->details_editor['text'] = 'No Guest Acees';
                }
                else{
                    $details = file_prepare_standard_editor($details, 'details', $editoroptions, $modulecontext, 'mod_facetoface', 'session', $sessions[$key]->id, true);
                }

                $sessions[$key]->cdetails = array(get_string('details', 'facetoface'), $details->details_editor['text']);
            }
 ...
 * @return array
 */
function facetoface_get_attendees($sessionid, $orderby = '') {
    global $CFG, $DB;
    ...
    // Task #20393 start
    if($orderby != ''){
        $orderby = 'ss.statuscode ASC,
    ISNULL(ss.display_order),
        ss.display_order ASC';
    }else{
        $orderby = 'ss.statuscode ASC, ss.timecreated ASC,
    ISNULL(ss.display_order),
        ss.display_order ASC';
    }
    // Task #20393 end
    
    ////Task #2314, changed query to fetch user's country column as well.
    $records = $DB->get_records_sql("
        ...
        ORDER BY
            $orderby
    ", array ($sessionid, MDL_F2F_STATUS_BOOKED, $sessionid, MDL_F2F_STATUS_APPROVED));
...
function facetoface_user_signup($session, $facetoface, $course, $discountcode,
                                ...
                                $notifyuser = true, $attendeesss=null,$conversion=null) {
    //$notificationtype = 2;
    ...
    if ($notifyuser) {
        ...
        switch ($newstatus) {
            case MDL_F2F_STATUS_BOOKED:
                $error = facetoface_send_confirmation_notice($facetoface, $session, $userid, $notificationtype, false, $conversion);
                break;

            case MDL_F2F_STATUS_WAITLISTED:
                $error = facetoface_send_confirmation_notice($facetoface, $session, $userid, $notificationtype, true, $conversion);
                break;
...
function facetoface_send_request_notice($facetoface, $session, $userid) {
    ...
    if (!$manageremail = facetoface_get_manageremail($session->id)) {
 ...
 *
 */
function facetoface_update_signup_status($signupid, $statuscode, $createdby, $note='', $grade=null, $isattended=null) {
    global $DB;
    $timenow = time();
    ...
    $signupstatus->isattended = $isattended;
 ...
 *  Thankyou Page
 */
function facetoface_thankyou_page(){
//Classroom Feature start
    $thankyou_page_html = '<div class="classroom-whole-section page-four4">
      <div class="thumsupimg"> <img src="registration-done-img.png"/> </div>
      <div class="classroom-fourpg-right-part"> <span class="rg-sucess-msg">'.get_string('thankyou_head','facetoface').'</span> <span class="rg-information-msg">'.get_string('thankyou_message','facetoface').'</span>
...
function facetoface_send_notice($postsubject, $posttext, $posttextmgrheading,
    ...
    $posthtml = ''; // FIXME.
    //Customization for no-reply@training.qsc.com
    if ($fromaddress = get_config('noreply@training.qsc.com', 'facetoface_fromaddress')) {
        $from = new stdClass();
        $from->maildisplay = true;
        $from->email = $fromaddress;
    } else {
    ...
    if (!empty($_SESSION['localSalInfo']) && $_SESSION['cntSeats'] != 2) {

        $posttextloctrnusermsg = facetoface_email_substitutions($facetoface->confirmationloctrnmsg, $facetoface->name, $facetoface->reminderperiod, $user, $session, $session->id);
        if ($notificationtype & MDL_F2F_TEXT) {
            $posthtml = $posttextloctrnusermsg;
            $user->mailformat = 1;
            $user->email = $user->username;
            if (!email_to_user($user, $from, $facetoface->confirmationloctrnsubject, $posttextloctrnusermsg, $posthtml)) {
                return 'error:cannotsendconfirmationuser';
            }
        }
    } else {
    ...
    $manageremail = facetoface_get_manageremail($session->id);
     if (strpos($manageremail, ',') !== FALSE)
        {
            $managerObj->email=$manageremail;
        }
    else{
    $managerObj = $DB->get_record('user', array('username'=>$manageremail),"*");
    }
   // $managerObj->email=$manageremail;
    
    if (!empty($posttextmgrheading) and !empty($manageremail)) {
       
        //Customization by sameer start
        $managertext = $posttextmgrheading;
        if ($manager = $DB->get_record('user', array('id'=>2),"*")) {
            // Leave out the ical attachments in the managers notification.
            $manager->mailformat = 1;
            $posthtml = $managertext;
            if(empty($_SESSION['localSalInfo'])){
                
                if (!email_to_user($manager, $from, $postsubject, $managertext, $posthtml)) {
                    return 'error:cannotsendconfirmationmanager';
                }

                if (!email_to_user($managerObj, $from, $postsubject, $managertext, $posthtml)) {
                  return 'error:cannotsendconfirmationmanager';
                }

                // $managerObj->email = "akshbk34@mailinator.com";
                // $subject='Monitor Automated Instructor '.$session->id;
                // if(!email_to_user($managerObj, $from, $subject, $managertext, $posthtml))
                // {
                //     return 'error:cannotsendconfirmationmanager';
                // }
            }
            ...
            $locSalArr = '';
            if(!empty($_SESSION['localSalInfo'])){
                $locSalArr = $_SESSION['localSalInfo'];

                $posttextloctrnmgrmsg = facetoface_email_substitutions($facetoface->confirmationloctrnmgr, $facetoface->name, $facetoface->reminderperiod, $user, $session, $session->id);

                if (!email_to_user($manager, $from, $facetoface->confirmationloctrnsubject, $posttextloctrnmgrmsg, $posthtml)) {
                    return 'error:cannotsendconfirmationmanager';
                }

                if (!email_to_user($managerObj, $from, $facetoface->confirmationloctrnsubject, $posttextloctrnmgrmsg, $posthtml)) {
                    return 'error:cannotsendconfirmationmanager';
                }

                $managerObj->email = "sameer.chourasia@beyondkey.com";
                $subject='Monitor Automated Instructors '.$session->id;
                if(!email_to_user($managerObj,$from, $subject, $posttextloctrnmgrmsg, $posthtml))
                {
                    return 'error:cannotsendconfirmationmanager';
                }

                for($i=0;$i<count($locSalArr['RepFirm']);$i++){
                    $manager->firstname = $locSalArr['RepFirm'][$i];
                    $manager->lastname = '';
                    $manager->email = $locSalArr['ContactEmail'][$i];
                    $manager->ccone = $locSalArr['CC1'][$i];
                    $manager->cctwo = $locSalArr['CC2'][$i];
                    $manager->ccthree = $locSalArr['CC3'][$i];

                    $posthtml = $posttextloctrnmgrmsg;
                    if (!email_to_user($manager, $from, $facetoface->confirmationloctrnsubject, $posttextloctrnmgrmsg, $posthtml)) {
                        return 'error:cannotsendconfirmationmanager';
                    }               
                } //end for
                unset($_SESSION['localSalInfo']);
            }// end if SESSION
        }// end if manager
    }
 ...
 * @returns string Error message (or empty string if successful)
 */
function facetoface_send_confirmation_notice($facetoface, $session, $userid, $notificationtype, $iswaitlisted, $conversion=null) {
    ...
    if (!$iswaitlisted) {
        ...
        $postsubject = $facetoface->confirmfromwaitlistedsubject;
            $posttext = $facetoface->confirmfromwaitlistedmsg;
    }
    } else {
 ...
 * @returns string Error message (or empty string if successful)
 */
function facetoface_send_cancellation_notice($facetoface, $session, $userid, $adminflag=false) {
    global $DB;
    if(!$adminflag){
    ...
    $postsubject = $facetoface->rescancelbyadminsubject;
    $posttext = $facetoface->rescancelbyadminmsg;
    $posttextmgrheading = '';
}
 ...
 */
function facetoface_get_manageremail($sessionid) {
    global $DB;
    //25 Aug Cust
    $fieldid = $DB->get_field('facetoface_session_field', 'id', array('shortname' => 'managersemail'));
    if ($fieldid) {
        return $DB->get_field('facetoface_session_data', 'data', array('sessionid' => $sessionid, 'fieldid' => $fieldid));
    } else {
...
function facetoface_cm_info_view(cm_info $coursemodule) {
    ...
    } else if ($sessions = facetoface_get_sessions($facetoface->id, null, null, null)) {
...
//Customized function sameer start
function facetoface_signup_session($courseid, $session, $showcapacity, $calendaroutput = false, $return = false, $hidesignup = false) {
    global $CFG, $DB;
    // display course overview files
    $sqlc3 = "SELECT * FROM {course} WHERE id = $courseid";
    $rsc3 = $DB->get_record_sql($sqlc3);
    $sessData[] = array("Logo", '');
    ...
    foreach ($customfields as $field) {
        ...
        if (!empty($customdata[$field->id])) {
            if (CUSTOMFIELD_TYPE_MULTISELECT == $field->type) {
                ...
                /* User Profile - start by pawan */
                $data = implode(', ', $values);

                /* User Profile - end by pawan */
            } else {
                $data = format_string($customdata[$field->id]->data);
            }
        }
        $sessData[] = array(str_replace(' ', '&nbsp;', format_string($field->name)), $data);
        $sessData[$field->name] = array(str_replace(' ', '&nbsp;', format_string($field->name)), $data);
        $_SESSION['signupform'][$field->name] = array(str_replace(' ', '&nbsp;', format_string($field->name)), $data);
    }
    ...
    if ($session->datetimeknown) {
        ...
        foreach ($session->sessiondates as $date) {
            ...
            /* Customization for date formate start by - pawan 27Nov */
            $timestartStr = $date->timestart;
            $timefinishStr = $date->timefinish;
            ...
            /* Customization for date formate end by - pawan 27Nov  */
            $timestart = date('h:i A', $date->timestart);
            $timefinish = date('h:i A', $date->timefinish);
            $html = "$timestart &ndash; $timefinish";
        }

        if ($dateend != '') {
            $strDate = $datestart . '- ' . $dateend;
            $sessData[] = array('Date', $strDate);
        } else {
            $sessData[] = array('Date', $datestart);
        }
        $sessData[] = array($strdatetime, $html);
    } else {
        // $sessData[] = array($strdatetime, html_writer::tag('i', get_string('wait-listed', 'facetoface')));
    }
    ...
    if ($showcapacity) {
        if ($session->allowoverbook) {
            //  $sessData[] = array(get_string('capacity', 'facetoface'), $session->capacity . ' ('.strtolower(get_string('allowoverbook', 'facetoface')).')');
        } else {
            $sessData[] = array(get_string('capacity', 'facetoface'), $session->capacity);
        }
    } else if (!$calendaroutput) {
        $sessData[] = array(get_string('seatsavailable', 'facetoface'), max(0, $placesleft));
    }
    ...
    if ($facetoface->approvalreqd) {
        $sessData[] = array('', get_string('sessionrequiresmanagerapproval', 'facetoface'));
    }

    // Display waitlist notification.
    if (!$hidesignup && $session->allowoverbook && $placesleft < 1) {
        $sessData[] = array('', get_string('userwillbewaitlisted', 'facetoface'));
    }

    if (!empty($session->duration)) {
        $sessData[] = array(get_string('duration', 'facetoface'), format_duration($session->duration));
    }
    if (!empty($session->normalcost)) {
        $sessData[] = array(get_string('normalcost', 'facetoface'), format_cost($session->normalcost));
    }
    if (!empty($session->discountcost)) {
        $sessData[] = array(get_string('discountcost', 'facetoface'), format_cost($session->discountcost));
    }
    if (!empty($session->details)) {
//Commented default section
//        $details = clean_text($session->details, FORMAT_HTML);
//        $sessData[] = array(get_string('details', 'facetoface'), $details);
//Customization Sameer Start
        //$moduleid = $DB->get_field('modules', 'id', array('name'=>'facetoface'));
        //$cmid = $DB->get_field('course_modules', 'id', array('module'=>$moduleid));
        $cm = get_coursemodule_from_instance('facetoface', $facetoface->id, $courseid);
        $modulecontext = context_module::instance($cm->id);
        $editoroptions = array(
            'noclean' => false,
            'maxfiles' => -1,
        ...
        $details->detailsformat = FORMAT_HTML;
        $details = file_prepare_standard_editor($details, 'details', $editoroptions, $modulecontext, 'mod_facetoface', 'session', $session->id);

        $sessData[] = array(get_string('details', 'facetoface'), $details->details_editor['text']);
    //Customization Sameer End
    }
    ...
    if ($trainerroles) {
        ...
        foreach ($trainerroles as $role => $rolename) {
            ...
            $sessData[] = array($rolename, implode(', ', $trainernames));
        }
    }

    $sessData["ts"] = array("timestart", $session->sessiondates);

    return $sessData;
}
//Customized function sameer end
...
function facetoface_print_session($session, $showcapacity, $calendaroutput=false, $return=false, $hidesignup=false) {
    ...
    if ($session->sessiondates[0]->timestart == "2481889900" && $session->sessiondates[0]->timefinish == "2481889900") {
        ...
        $session->datetimeknown = 1;
        $filedflag = 10000000000000;
    }
    
    foreach ($customfields as $field) {
        $data = '';
        if ($field->id != 8 && $field->id != 9 && $field->id != $filedflag) {
            if (!empty($customdata[$field->id])) {
                if (CUSTOMFIELD_TYPE_MULTISELECT == $field->type) {
                    $values = explode(CUSTOMFIELD_DELIMITER, format_string($customdata[$field->id]->data));
                    /* User Profile - start by pawan */
            ...
            if ($data) {
                ////Start - Task 2369 changes
                $fieldTitleToShow = '';
                if ($field->id == 10) {
                    $fieldTitleToShow = get_string('time_length', 'facetoface');
                } else if ($field->id == 1) {
                    $fieldTitleToShow = get_string('location', 'facetoface');
                } else {
                    $fieldTitleToShow = format_string($field->name);
                }
                ////End - Task 2369 changes
                $table->data[] = array(str_replace(' ', '&nbsp;', $fieldTitleToShow), $data);
            }
        }
    }
    ...
    if ($session->datetimeknown) {
        ...
        foreach ($session->sessiondates as $date) {
            ...
            $timestart = date("d F Y, h:i A",$date->timestart);
            $timefinish = date("d F Y, h:i A",$date->timefinish);
            $html .= "$timestart &ndash; $timefinish";
        }
 ...
 */
// - --  --Start -New Feature Request: Editable Instructor Name Field- add $contextId- - ----- ----//
function facetoface_add_customfields_to_form_new(&$mform, $customfields, $alloptional=false, $contextId ='') {
    global $CFG, $DB;
    // - --  --End -New Feature Request: Editable Instructor Name Field- - - ----- ----//
    foreach ($customfields as $field) {
        ...
        $options = array();
        //US 3072 start       
        if($fieldname =="custom_countries")
        {
            $choices = get_string_manager()->get_list_of_countries();
            if(!empty($choices))
            {
                foreach ($choices as $ckey => $cname) {
                   $options[$ckey] =  $cname;
                }
            }
        }
        ...
        // - --  --Start -New Feature Request: Editable Instructor Name Field- - - ----- ----//
        if($fieldname =="custom_qscinstructorname" OR $fieldname =="custom_repinstructorname"){
            if($fieldname =="custom_qscinstructorname")
            {
                $enrolled_QSCInst= $DB->get_records("enrolled_instructor", array('shortname'=>"qscinstructorname",'contextid'=> $contextId));
                if(!empty($enrolled_QSCInst))
                {
                    foreach ($enrolled_QSCInst as $key => $QSCvalue) {
                       $options[$QSCvalue->possiblevalues] =  $QSCvalue->possiblevalues;
                    }
                }
            }else if($fieldname =="custom_repinstructorname")
            {
                $enrolled_RepInst= $DB->get_records("enrolled_instructor", array('shortname'=>"repinstructorname",'contextid'=> $contextId));
                if(!empty($enrolled_RepInst))
                {
                    foreach ($enrolled_RepInst as $key => $Repvalue) {
                       $options[$Repvalue->possiblevalues] =  $Repvalue->possiblevalues;
                    }
                }
            }
        }else{
           ...
           /* Customization start */
                if (!empty($v)) {
                    if ($v === "Local Classes") {
                      $options[$v] = get_string('local_classess','facetoface');  
                    }
                    else if ($v == "Inside QSC") {
                      $options[$v] = get_string('inside_qsc','facetoface');  
                    }
                    else if ($v == "Outside QSC") {
                      $options[$v] = get_string('outside_qsc','facetoface');  
                    }
                    else
                    $options[$v] = $v;
                }
        ...
        switch ($field->type) {
            case CUSTOMFIELD_TYPE_TEXT:
                ...
                break;
        case CUSTOMFIELD_TYPE_TEXTAREA:
                $mform->addElement('textarea', $fieldname, $field->name);
                break;
...
// US #6665
function remove_waitlisted_users($s) {
    ...
    if ($waitlistusers) {
        foreach ($waitlistusers as $waitlistuser) {
            ...
            $userdetail = $DB->get_record('user', array('id' => $waitlistuser->userid));
            delete_signup_users($userdetail, $session, $facetoface, $waitlistuser->id);
        }
    } else {
        return print_error('error:nowaitlistuser', 'facetoface');
    }
    return true;
}

// US 6665
function delete_signup_users($user, $session, $facetoface, $signupid) {

    global $DB;

    $titlefieldtype = $DB->get_field('facetoface_session_field', 'id', array('shortname' => 'title',));
    $locationfieldtype = $DB->get_field('facetoface_session_field', 'id', array('shortname' => 'Location',));
    $streetfieldtype = $DB->get_field('facetoface_session_field', 'id', array('shortname' => 'Street',));

    $sesstiontitle = $DB->get_field('facetoface_session_data', 'data', array('fieldid' => $titlefieldtype, 'sessionid' => $session->id));
    $sesstionlocation = $DB->get_field('facetoface_session_data', 'data', array('fieldid' => $locationfieldtype, 'sessionid' => $session->id));
    $sesstionstreet = $DB->get_field('facetoface_session_data', 'data', array('fieldid' => $streetfieldtype, 'sessionid' => $session->id));
    $sessiondates = $DB->get_record('facetoface_sessions_dates', array('sessionid' => $session->id));

    if (empty($facetoface->clearwaitlistsubject)) {
        $subject = get_string('setting:defaultclearwaitlistsubject', 'facetoface');
    } else {
        $subject = $facetoface->clearwaitlistsubject;
    }

    $msgsubject = $subject;
    $msgsubject = str_replace(get_string('placeholder:sessiontitle', 'facetoface'), $sesstiontitle, $msgsubject);

    if (empty($facetoface->clearwaitlistmessage)) {
        $message = get_string('setting:defaultclearwaitlistmessage', 'facetoface');
    } else {
        $message = $facetoface->clearwaitlistmessage;
    }

    $msgbody = $message;
    $msgbody = str_replace(get_string('placeholder:firstname', 'facetoface'), $user->firstname, $msgbody);
    $msgbody = str_replace(get_string('placeholder:sessiontitle', 'facetoface'), $sesstiontitle, $msgbody);
    $msgbody = str_replace(get_string('placeholder:sessiondate', 'facetoface'), date('d-m-Y', $sessiondates->timestart) . '-' . date('d-m-Y', $sessiondates->timefinish), $msgbody);
    $msgbody = str_replace(get_string('placeholder:time', 'facetoface'), date('H:i:s', $sessiondates->timestart), $msgbody);
    $msgbody = str_replace(get_string('placeholder:location', 'facetoface'), $sesstionlocation, $msgbody);
    $msgbody = str_replace(get_string('placeholder:street', 'facetoface'), $sesstionstreet, $msgbody);
    ...
    if ($user->username) {
        ...
        $emailobj->fromuserid = 2;
        $emailobj->fromemail = $fromuser->email;
        $emailobj->touserid = $user->id;
        $user->email = ($user->email) ? $user->email : $user->username;
        $emailobj->toemail = $user->email;
        $emailobj->subject = $emailsubject;
        $emailobj->emailbody = $emailbody;
        $emailobj->sentdate = time();
        $emailobj->sessionid = $session->id;
        $emailobj->facetofaceid = $facetoface->id;
        $emailobj->courseid = $facetoface->course;
        $emailobj->usercreated = 2;
        $emailobj->status = 0;
        $emailobj->signupid = $signupid;
        $emailobj->timecreated = time();
    }

    $params = array('signupid' => $signupid, 'sessionid' => $session->id);

    if (!$DB->record_exists('local_clearwaitlist_emails', $params)) {
        $result = $DB->insert_record('local_clearwaitlist_emails', $emailobj);
        if ($result) {
            $sqlstatusdel = "DELETE FROM {facetoface_signups_status} WHERE signupid =" . $signupid . "";
            $sqldelres1 = $DB->execute($sqlstatusdel);
            $sqlsignupdel = "DELETE FROM {facetoface_signups} WHERE id =" . $signupid . " AND userid =" . $user->id;
            $sqldelres2 = $DB->execute($sqlsignupdel);
        }
    }
}
