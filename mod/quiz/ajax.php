...
require_login();
if($_POST['msg_body'] == ''){
echo json_encode("Please add message.");
if (isguestuser()) { echo json_encode("Your session is destroyed. Please login again.");}
die;
}
if (isguestuser()) { echo json_encode("Your session is destroyed. Please login again.");}
else{
$PAGE->set_url('/mod/quiz/ajax.php');
$PAGE->set_context(context_system::instance());

//Mail feature
$cm = get_coursemodule_from_id('quiz', optional_param('coursemoduleid', null, PARAM_INT)); //print_r($cm); exit("Success1");
$course = $DB->get_record('course', array('id' => $cm->course));

$pattern = "#<\s*?span\b[^>]*>(.*?)</span\b[^>]*>#s";
//$pattern = '/(?<=lang\=\"en\"\>).*(?=\<\/span\>)/';
...
$coursemoduleid = optional_param('coursemoduleid', null, PARAM_INT); 

$msgbody = optional_param('msg_body', null, PARAM_TEXT);
$outcome[0] = $_POST['msg_body'];
$outcome[1] = $_POST['quizid'];
$from = $USER->email;
$messagehtml = text_to_html($_POST['msg_body'], false, false, true);
...
//email_to_user($recipient, $eventdata->userfrom, $eventdata->subject, $eventdata->fullmessage,$eventdata->fullmessagehtml, $attachment, $attachname, true, $replyto, $replytoname);

/*if(mail($admin->email, $coursename." Reset Quiz Attempt for ".$USER->firstname." ".$USER->lastname, $_POST['msg_body'], $from)){
...
if(email_to_user($adminObj, $from, $coursename." Reset Quiz Attempt for ".$USER->firstname." ".$USER->lastname, $msgbody)){
	echo json_encode("Message Sent Successfully");
}
else{
