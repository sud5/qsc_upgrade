...
if (has_capability('tool/mergeusers:mergeusers', context_system::instance())) {
    ...
    if (!$ADMIN->locate('tool_mergeusers')) {
            $ADMIN->add('courses',
            new admin_category('privatecoursegroup', get_string('privatecourse', 'moodle')));

         $ADMIN->add('privatecoursegroup', new admin_externalpage('privatecoursegroup_create', get_string('create_group', 'moodle'), "$CFG->wwwroot/admin/privatecoursegroup/add_tag.php"));
  
    
         $ADMIN->add('privatecoursegroup', new admin_externalpage('uploaduser_tag', get_string('uploaduser_tag', 'moodle'), "$CFG->wwwroot/admin/tool/uploaduser/tag.php"));
    //-- - - - End - PrivateCoursePhase2  Nav  for Private Course Navigation End  
        $ADMIN->add('accounts',
