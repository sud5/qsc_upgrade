...
// Start update - dashboard_admin_grader - prev
    if($USER->usertype == 'grader' || $USER->usertype == 'graderasadmin'){
        ...
        redirect(new moodle_url($CFG->wwwroot.'/my/dashboard_admin_grader_new.php'));
    }else{
...
<!-- Grader as a Manager Code Start -->
     <div class="three_tabs_newdashboard" style="position:absolute !important;margin-top: 0px !important;">
...
//     if($USER->id == 12301){
//         unset($rsStudentRecordArray);
//         unset($rsStudentRecordCommentObj);
//           $sqlUser_AssignGrader_AssignSubmission ="SELECT MAX(gr.id) as agid,u.id,gr.grader_id,u.picture,u.firstname,u.lastname,u.firstnamephonetic,u.lastnamephonetic,u.institution, u.middlename,u.alternatename,u.imagealt,u.email, u.id as userid, gr.exam_id as course_modules_id, cm.course as course_id, cm.instance as assignmentid, c.fullname as course_name, c.visible AS course_visible, s.status as status, s.id as assign_submission_id, s.timecreated as firstsubmission, s.timemodified as timesubmitted, s.attemptnumber as attemptnumber, g.id as gradeid, g.grade as grade, g.timemodified as timemarked, g.timecreated as firstmarked FROM mdl_assign_graders gr RIGHT JOIN mdl_user u ON u.id = gr.student_id AND u.deleted != 1 LEFT JOIN mdl_course_modules cm ON gr.exam_id = cm.id AND cm.module =1 LEFT JOIN mdl_assign_submission s ON u.id = s.userid AND s.assignment = cm.instance LEFT JOIN mdl_course c ON c.id = cm.course LEFT JOIN mdl_assign_grades g ON u.id = g.userid AND g.assignment = cm.instance AND g.attemptnumber = s.attemptnumber WHERE s.status IS NOT NULL AND s.latest =1 AND s.status != 'new' AND gr.grader_id=".$USER->id." AND (g.grade IS NULL OR g.grade < (SELECT gi.gradepass from mdl_grade_items gi where gi.iteminstance=cm.instance AND gi.itemmodule='assign')) group by u.id, cm.course ORDER BY s.timemodified DESC";
//     $rsStudentRecordArray = $DB->get_records_sql($sqlUser_AssignGrader_AssignSubmission);
...
// //echo "<pre>"; print_r($rsStudentRecordArray); exit;
...
<div class="certification_in_progress notification-panel">
    <h4>Notifications</h4>
        ...
        <style type="text/css">.row-fluid .span9{ width: 100% !important;  }</style>
        ...
        <table id='studentDataTable' class="table-bordered graderpg_cls">
			...
			if(is_enrolled($coursecontextsecond, $rsStudentCommentRecord->userid, '', false)){
                ...
                ?>
                        ...
                        <td class="">
                            <span> <?php //echo date("Ymd",$rsComments->timecreated); 
