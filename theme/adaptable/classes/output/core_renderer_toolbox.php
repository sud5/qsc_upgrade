...
trait core_renderer_toolbox {
    ...
    public function user_profile_menu() {
        ...
        if (is_siteadmin($USER)) {
            $usermenuitems[] = array(false, false, $CFG->wwwroot.'/mod/data/view.php?id=225', 'ZIPCODE',
            \theme_adaptable\toolbox::getfontawesomemarkup('cog'));
        }
        ...
        $usermenuitems[] = array('enablecalendar', false, $CFG->wwwroot.'/calendar/sessionview.php',
        ...
        if(!is_siteadmin($USER) && ($_SESSION['REALUSER']->username == 'admin' || $_SESSION['REALUSER']->usertype == 'graderasadmin') ){
        $usermenuitems[] = array(false, false, $CFG->wwwroot.'/login/logout_user.php', 'LOG OUT AS '.strtoupper($USER->firstname." ".$USER->lastname),
...
EOT;
    ...
    public function navbarsdasd() {
        ...
        foreach ($items as $item) {
            ...
            if ($start) {
                ...
                $breadcrumbs .= html_writer::link(new moodle_url('/my'), get_string('dashboard', 'theme_adaptable'));
                $breadcrumbs .= '</li>';
                $start = false;
    ...
    public function get_logo($currenttopcat, $shownavbar) {
        ...
        if (!empty($logosetarea)) {
            ...
            $responsivelogo = "d-lg-inline-block";
            $logomarkup = '<div class="pr-3 bd-highlight'.$responsivelogo.'">';
    ...
    public function blocks($region, $classes = array(), $tag = 'aside', $fakeblocksonly = false) {
        ...
        if ((!empty($output)) && ($region == 'side-post')) {
            $output .= html_writer::tag('div',
                html_writer::tag('span', 'administration', array('class' => 'administration-block-left', 'aria-hidden' => 'true')),
    ...
    public function dockpanel(){
        global $OUTPUT;
        return $OUTPUT->render_from_template('theme_adaptable/dock', []);
    }
}
