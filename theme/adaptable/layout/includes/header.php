...
<div id="page-wrapper">
    ...
    if ((!isloggedin() || isguestuser()) && ($PAGE->pagetype != "login-index")) {
        if ($PAGE->theme->settings->displaylogin != 'no') {
           ...
           // $headercontext['loginoruser'] = '<li class="nav-item">'.
            ...
            //$headercontext['loginoruser'] = '<li class="nav-item">'. auth_googleoauth2_display_buttons() .'</li>';
        } else {
            $headercontext['loginoruser'] = '';
        }
    } else {
        ...
        if ((isloggedin()) && ($PAGE->pagelayout != 'secure')) {
            ...
            // User name.
            if(!is_siteadmin() && ($_SESSION['REALUSER']->username == 'admin' || $_SESSION['REALUSER']->usertype == 'graderasadmin') ){
                $username = '';
                if ($_SESSION['REALUSER']->username == 'admin') {
                    $username = format_string(strtoupper($_SESSION['REALUSER']->username.' USER'));
                } elseif($_SESSION['REALUSER']->usertype == 'graderasadmin') {
                    $username = format_string(strtoupper($_SESSION['REALUSER']->firstname.' '. $_SESSION['REALUSER']->lastname. ' USER'));
                } else {
                    $username = format_string(strtoupper($_SESSION['REALUSER']->username.' USER'));
                }
    
                $loginasuser = format_string(fullname($USER));
            }else{
    ...
    if ($adaptableheaderstyle == "style1") {
        ...
        include_once("/var/www/html/moodle42/theme/meline29/layout/includes/top_menu_new.php");
        echo $OUTPUT->render_from_template('theme_adaptable/headerstylerightone', $headercontext);
