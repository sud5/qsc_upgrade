...
if(isset($getInCompPreCourseId)){
  ...
  unset($getInCompPreCourseId);
}
elseif(isset($keyCD)) {
  $flagpopup=1;
  $courseObjData = $DB->get_record('course', array('id'=>$keyCD->courseinstance),'fullname');
  if($keyCD->courseinstance != 7) {
    $liCourseList = '<li><a href="/course/view.php?id='.$keyCD->courseinstance.'">'.format_string($courseObjData->fullname).'</a></li>';
  }
  else {
    //// Task 2369, converted hardcoded text to macros
    $liCourseList = '<li><a href="/course/view.php?id='.$keyCD->courseinstance.'">'.format_string($courseObjData->fullname).'</a> <span style="color:#333"> '.get_string("prereqseven").' </span> <a href="/course/view.php?id=49">'.get_string("prereqeight").'</a></li>';
  }
}
//1069 end
?>
<?php if($flagpopup==1){ ?>
 <!-- Modal Start For Exit Exam Design Page-->
                <div class="modal fade modal-lgg" id="memberModal"  tabindex="-1" role="dialog" aria-labelledby="memberModalLabel" aria-hidden="true" style="top:20%;">
                    ...
                    <div class="modal-content">
                      ...
                      <div class="modal-body">
                        <div class="prereq_comp_page_main">
                          <div class="prereq_comp_page_bottom_full_section" style="margin:10px 0 10px 5px !important;">
                            ...
                            <div class="bottom_section">
                              ...
                              <div class="prereq_comp_page_full_right_secion">
                                <!-- PP2 start -->
                                <p>
                                <!--Start - Task 2369, replaced hardcoded text with placeholder "prereqsix" -->
                                <?=get_string("prereqsix")?> 
                                <!--End - Task 2369, replaced hardcoded text with placeholder "prereqsix" -->
                                <strong><?=format_string($course->fullname)?></strong> <?=get_string("prereqtwo")?>:
                                </p>
                                <!-- PP2 end -->
                                <ul>
                                  <?php 
                                ...
                                <!-- TEXT course name should be dynamic -->
                                <?php         
                                  $a->coursename = format_string($course->fullname);
                                  $a->precoursename = format_string($courseObjData->fullname);
                                ...
                                ?>
                                <?php //change-1 start ?>
                                <p><?=get_string("prereqthree",'',$a)?></p>
                                <?php //change-1 end ?>
                                  <!-- PP2 start -->
                                  <?php if(!isset($_REQUEST['as'])){ 
                                      $fordwardURL ="/mod/facetoface/view.php?f=".$f."&flags=1kqwe";?>
                                      <a data-dismiss="modal" href = 'JavaScript:Void(0);';"><?=get_string("prereqfour",'',$b)?></a>
                                      <?php
                                  }else{ 
                                      $fordwardURL ="/mod/facetoface/view.php?f=".$f."&as=".$_REQUEST['as']."&sc=".$_REQUEST['sc'];
                                      ?>
