...
function core_myprofile_navigation(core_user\output\myprofile\tree $tree, $user, $iscurrentuser, $course) {
    ...
    if (is_mnet_remote_user($user)) {
        ...
        $node = new core_user\output\myprofile\node('contact', 'firstname', get_string('name'), null, null, $user->firstname." ".$user->lastname);
        $tree->add_node($node);

       /*$node = new core_user\output\myprofile\node('contact', 'lastname', get_string('lastname'), null, null, $user->lastname);
        $tree->add_node($node);*/
        //$tree->add_node($firstLastname." ".$nodeLastname);

        /* User profile end */

        $node = new core_user\output\myprofile\node('contact', 'institution', get_string('company'), null, null, $user->institution);
        $tree->add_node($node);
    ...
    //if (isset($identityfields['username']) && $user->username) {
        $node = new core_user\output\myprofile\node('contact', 'email', get_string('email'), null, null, '<div class = "username">'.$user->username.'</div>');
        $tree->add_node($node);
    //}
    ...
    //if (!isset($hiddenfields['country']) && $user->country) {
        //- - - - - - -- Start -Country and state code Change-  - - - - -//
        // $node = new core_user\output\myprofile\node('contact', 'country', get_string('country'), null, null,
        //         get_string($user->country, 'countries'));
        // $tree->add_node($node);

        if (($USER->id != 2 && $USER->usertype != 'mainadmin' && $USER->usertype != 'graderasadmin') || strlen($user->country) == 3) {
             $node = new core_user\output\myprofile\node('contact', 'country', get_string('country'), null, null,get_string($user->country, 'countries'));
            $tree->add_node($node);
        }else{
        $countrySql= $DB->get_record_sql("SELECT * FROM `country` WHERE iso ='".$user->country."' LIMIT 0,1 ");
            if(!empty($countrySql))
            {
                $node = new core_user\output\myprofile\node('contact', 'country', get_string('country'), null, null,get_string($countrySql->iso3, 'countries'));
                $tree->add_node($node);
            }else{
                $node = new core_user\output\myprofile\node('contact', 'country', get_string('country'), null, null,get_string($user->country, 'countries'));
                $tree->add_node($node);
    ...
    //if (!isset($hiddenfields['zip']) && $user->zip) {
        $node = new core_user\output\myprofile\node('contact', 'zip', get_string('zip'), null, null,
                $user->zip);
        $tree->add_node($node);
    //}
    ...
    if (!empty($user->url) && !isset($hiddenfields['webpage'])) {
        $url = $user->url;
        if (strpos($user->url, '://') === false) {
            $url = 'http://'. $url;
        }
    ...
    // Printing tagged interests. We want this only for full profile.
    if (!empty($CFG->usetags) && empty($course)) {
        if ($interests = core_tag_tag::get_item_tags_array('user', $user->id, null) ) {
            $node = new core_user\output\myprofile\node('contact', 'interests', get_string('interests'), null, null, $interests);
            $tree->add_node($node);
        }
    }
    ...
    if (!empty($user->icq) && !isset($hiddenfields['icqnumber'])) {
        $imurl = new moodle_url('http://web.icq.com/wwp', array('uin' => $user->icq) );
        $iconurl = new moodle_url('http://web.icq.com/whitepages/online', array('icq' => $user->icq, 'img' => '5'));
        $statusicon = html_writer::tag('img', '',
                array('src' => $iconurl, 'class' => 'icon icon-post', 'alt' => get_string('status')));
    ...
    if (!empty($user->skype) && !isset($hiddenfields['skypeid'])) {
        $imurl = 'skype:'.urlencode($user->skype).'?call';
        $iconurl = new moodle_url('http://mystatus.skype.com/smallicon/'.urlencode($user->skype));
        if (is_https()) {
            // Bad luck, skype devs are lazy to set up SSL on their servers - see MDL-37233.
    ...
    if (!empty($user->yahoo) && !isset($hiddenfields['yahooid'])) {
        $imurl = new moodle_url('http://edit.yahoo.com/config/send_webmesg', array('.target' => $user->yahoo, '.src' => 'pg'));
        $iconurl = new moodle_url('http://opi.yahoo.com/online', array('u' => $user->yahoo, 'm' => 'g', 't' => '0'));
        $statusicon = html_writer::tag('img', '',
            array('src' => $iconurl, 'class' => 'iconsmall icon-post', 'alt' => get_string('status')));
    ...
    if (!empty($user->aim) && !isset($hiddenfields['aimid'])) {
        $imurl = 'aim:goim?screenname='.urlencode($user->aim);
        $node = new core_user\output\myprofile\node('contact', 'aimid', get_string('aimid'), null, null,
            html_writer::link($imurl, s($user->aim)));
        $tree->add_node($node);
    }
    if (!empty($user->msn) && !isset($hiddenfields['msnid'])) {
        $node = new core_user\output\myprofile\node('contact', 'msnid', get_string('msnid'), null, null,
            s($user->msn));
        $tree->add_node($node);
    }
