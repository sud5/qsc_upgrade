...
function removeCourseForBlankCert($rbcVal){
    ...
    $url = "$instance_url/services/data/v50.0/query?q=" . urlencode($query);
...
//Additional buggy
    ...
    function show_module_completion_status($userid, $courseid) {
        ...
        foreach ($sectionDataRs as $key) {
            ...
            $completeCoursesModulesDataSQL = "SELECT cmc.id,cmc.userid,cmc.coursemoduleid FROM {course_modules} cm JOIN {course_modules_completion} cmc ON cmc.coursemoduleid = cm.id WHERE cm.course =$courseid and cmc.userid= $userid AND cmc.coursemoduleid IN($sequence) AND cmc.completionstate != 3 and cm.course < 128";
    ...
    function show_contact_id_by_email($username) {
        global $USER;
        $access_token = $_SESSION['access_token'];
        $instance_url = $_SESSION['instance_url'];

        $query = "SELECT id, email from Contact where email='".$username."'";
...
//echo "<pre>22sss22"; print_r($sfdc_contact_id_arr); 
      ...
      // foreach($sfdc_contact_id_arr['records'] as $key){
            $id = $sfdcuniqueid;
            $url = "$instance_url/services/data/v50.0/sobjects/Contact/$id";              
...
//echo "<pre>22sss22"; print_r($sfdc_contact_id_arr); 
    ...
    function set_sfdc_curl($content, $url, $access_token){
        ...
        if ( $status != 201 ) {
            //$insert_header = "INSERT INTO mdl_different_email (name, certificateid, timecreated, timeexpired) VALUES (".$sqlSCILObjData->userid.", ".$keyCertVal->id.", '".$keyCertVal->name."',0, '".$sqlSCILObjData->timecompletion."', '".$certExpiredDate."')";
            //$DB->execute($insert_header);
            $jenc = json_decode($content);
           // print_r($jenc->Name);
            if(!empty($jenc->certified__c)){
                echo "\n inn \n";
                $certname = $jenc->Name;
                $email = $jenc->email__c;
                $course_id = $jenc->course_sfdc_id__c;
                $contact_id = $jenc->contact_sfdc_id__c;
                $cert_status = $jenc->certified__c;
                $insert_head = "INSERT INTO mdl_different_email (id, certname, email, course_id, contact_id,cert_status) VALUES (NULL, '".$certname."', '".$email."', '".$course_id."', '".$contact_id."', '".$cert_status."')";
                $DB->execute($insert_head);
                $json_response=0;
            }
           // echo "\n json response \n";
    ...
    function sfdc_delete_course_by_Id($id) {
        ...
        $url = "$instance_url/services/data/v50.0/sobjects/Course__c/$id";
    ...
    //Delete quiz details by Id - customo    
    function sfdc_delete_quiz_by_Id($id) {
        $access_token = $_SESSION['access_token'];
        $instance_url = $_SESSION['instance_url'];

        $url = "$instance_url/services/data/v50.0/sobjects/Assessment__c/$id";
        ...
        //echo $status." Test Delete Course"; //print_r($json_response); //exit("I AM In Deleted Assessment section");
        if ( $status != 204 ) {
            return $status;
        }
        curl_close($curl);
    ...
    //Delete quiz details end - customo
    ...
    function sfdc_delete_classroom_by_course_classroom_id($id)
    {
        ...
        $url = "$instance_url/services/data/v50.0/sobjects/Classroom__c/$id";
    ...
    //Update course object's date of training field
    function sfdc_update_dot($id, $dot) {

        $access_token = $_SESSION['access_token'];
        $instance_url = $_SESSION['instance_url'];

        $url = "$instance_url/services/data/v50.0/sobjects/Course__c/$id";
        $content = json_encode(array("date_of_training__c"=>$dot)); 
    ...
    //
      //  echo "<tr><td>". $record['Contact__c'] . "</td><td>" .$record['Name'] . "</td><td>" . $record['course_lms_id__c'] . "</td><td>" . $record['course_version_type__c'] . "</td></tr>";
        ...
        //Display course details
    function sfdc_show_courses_by_contactid($contact_id) {

        $access_token = $_SESSION['access_token'];
        $instance_url = $_SESSION['instance_url'];
        $query = "SELECT name,id,contact__c,course_lms_id__c,course_version_type__c from Course__c where contact__c='$contact_id'";
    ...
    function sfdc_create_course($contact_id, $course_name, $course_lms_id, $course_type,$dot,$mod_status='') {
        ...
        $url = "$instance_url/services/data/v50.0/sobjects/Course__c/";
...
//exit;
    ...
    function sfdc_create_onlinecourse($course_sfdc_id, $date_of_training){
        ...
        $url = "$instance_url/services/data/v50.0/sobjects/online_course__c/";
    ...
    function show_classroom_by_course_id($course_sfdc_id) {
     ...
     //#20122
    function sfdc_account_by_contact($sfdc_contact_id){
        $access_token = $_SESSION['access_token'];
        $instance_url = $_SESSION['instance_url'];

        $query = "SELECT AccountId from Contact where id='$sfdc_contact_id'";
    ...
    function sfdc_create_sessionattendee($acc_sfdc_id=null,$cr=null,$sfdc_contact_id,$country=null,$dr=null,$manager_email=null,$session_lms_id,$signupstatus,$session_sfdc_id,$session_cancel_time=null,$session_enrolled_date){
        $access_token = $_SESSION['access_token'];
        $instance_url = $_SESSION['instance_url'];

        $url = "$instance_url/services/data/v50.0/sobjects/Session_Attendee__c/";
        //Account__c
        //Cancellation_Reason__c   (sfdc_create_classroom_cancel_registration)
        //Contact__c
        //Country__c
        //Diet_Restriction__c
        //Email__c
        //LMS_Session_ID__c
        //Registration_Status__c (Reserved,Cancelled,Attended)
        //Session__c
        //Time_Cancelled__c
        //Time_Signed_Up__c
...
//     }
        ...
        $content = json_encode(array("Account__c"=>$acc_sfdc_id,"Cancellation_Reason__c" => $cr,"Contact__c"=>$sfdc_contact_id,"Country__c"=>$country,"Diet_Restriction__c"=> $dr,"Email__c"=>$manager_email,"Time_Signed_Up__c"=>$session_enrolled_date,"Time_Cancelled__c"=>$session_cancel_time,"Registration_Status__c" => $signupstatus,"Session__c" => $session_sfdc_id,"LMS_Session_ID__c"=>$session_lms_id));
...
//#20122 end
    ...
    function sfdc_sess_att_sf_exist($sfdc_contact_id, $session_sfdc_id){
        global $USER;
        $access_token = $_SESSION['access_token'];
        $instance_url = $_SESSION['instance_url'];

        $query = "SELECT id from Session_Attendee__c where Contact__c='".$sfdc_contact_id."' and Session__c='".$session_sfdc_id."'";
    ...
    function sfdc_sess_att_sf_lms_id_exist($session_sfdc_id){
        global $USER;
        $access_token = $_SESSION['access_token'];
        $instance_url = $_SESSION['instance_url'];

        $query = "SELECT LMS_Session_ID__c from Session_Attendee__c where Session__c='".$session_sfdc_id."'";
        ...
        // print_r($response);
        if(isset($response['records'])){
            return $response['records'][0]['LMS_Session_ID__c'];
        }
        else{
            return 0;
        }
    ...
    function sfdc_sess_att_sf_cancel_time_exist($session_sfdc_id){
        global $USER;
        $access_token = $_SESSION['access_token'];
        $instance_url = $_SESSION['instance_url'];

        $query = "SELECT Time_Cancelled__c from Session_Attendee__c where Session__c='".$session_sfdc_id."'";
        ...
        // print_r($response);
        if(isset($response['records'])){
            return $response['records'][0]['Time_Cancelled__c'];
        }
        else{
            return 0;
        }
    ...
    function sfdc_sess_date_sf_exist(){
        global $USER;
        $access_token = $_SESSION['access_token'];
        $instance_url = $_SESSION['instance_url'];

        $query = "SELECT id from Session__c where Session_LMS_Id__c LIKE '%-%'";
    ...
    //SELECT Id,Name,session_end_date__c,session_start_date__c,Session_LMS_Id__c FROM Session__c WHERE Session_LMS_Id__c LIKE '%-%'
    
    function sfdc_update_sessionattendee($sess_att_sf_id=null,$acc_sfdc_id=null,$cr=null,$sfdc_contact_id=null,$region=null,$dr=null,$useremail,$session_lms_id,$signupstatus=null,$session_sfdc_id=null,$session_cancel_time=null,$session_enrolled_date=null){
          $access_token = $_SESSION['access_token'];
        $instance_url = $_SESSION['instance_url'];

        $url = "$instance_url/services/data/v50.0/sobjects/Session_Attendee__c/$sess_att_sf_id";
        
        $content = json_encode(array("Account__c"=>$acc_sfdc_id,"Cancellation_Reason__c" => $cr,"Country__c"=>$region,"Diet_Restriction__c"=> $dr,"Email__c"=>$useremail,"Time_Signed_Up__c"=>$session_enrolled_date,"Time_Cancelled__c"=>$session_cancel_time,"Registration_Status__c" => $signupstatus,"Session__c" => $session_sfdc_id,"LMS_Session_ID__c"=>$session_lms_id));
    ...
    #20122 add location variable
    function show_all_session_by_classroom_sfdc_id($classroom_sfdc_id) {
        $access_token = $_SESSION['access_token'];
        $instance_url = $_SESSION['instance_url'];

        $query = "SELECT id,session_start_date__c,session_end_date__c, name,location__c from Session__c where classroom_sfdc_id__c='$classroom_sfdc_id'";
    ...
    function sfdc_create_classroomcourse($course_sfdc_id, $classroom_name=null){
        ...
        $url = "$instance_url/services/data/v50.0/sobjects/Classroom__c/";
 ...
 //20122
    function show_sessionid_by_classidandsesslmsid($classroomcouse_sfdc_id, $session_lms_id){
        $access_token = $_SESSION['access_token'];
        $instance_url = $_SESSION['instance_url'];

        $query = "SELECT id from Session__c where classroom_sfdc_id__c='$classroomcouse_sfdc_id' and Session_LMS_Id__c='$session_lms_id'";
        
        $url = "$instance_url/services/data/v50.0/query?q=" . urlencode($query);  
        $json_response = get_sfdc_curl($query, $url, $access_token);    

        $response = json_decode($json_response, true);   
       // echo "\n lib"; print_r($response); exit;
        if($response)
    ...
    //0122
    function show_sessionid_by_classid($classroomcouse_sfdc_id){
        $access_token = $_SESSION['access_token'];
        $instance_url = $_SESSION['instance_url'];

        $query = "SELECT id from Session__c where classroom_sfdc_id__c='$classroomcouse_sfdc_id' and Session_LMS_Id__c=''";
...
//US #824 start
    ...
    function sfdc_create_classroom_session($classroom_sfdc_id, $classroom_type=null,$duration,$level_type,$location,$manager_email,$session_start_date,$session_end_date,$sn,$street,$session_Instructor,$session_enrolled_date,$signupstatus,$classroomtype=null,$session_lms_id=null){
        $access_token = $_SESSION['access_token'];
        $instance_url = $_SESSION['instance_url'];

        $url = "$instance_url/services/data/v50.0/sobjects/Session__c/";
...
$sn = substr(htmlspecialchars($sn), 0, 75);
  ...
  if(!empty($session_Instructor)){
        ...
        $content = json_encode(array("classroom_sfdc_id__c"=>$classroom_sfdc_id,"classroom_type__c" => $classroom_type,"duration__c"=>$duration,"level_type__c"=>$level_type,"location__c"=> $location,"manager_email__c"=>$manager_email,"session_start_date__c"=>$session_start_date,"session_end_date__c"=>$session_end_date,"street__c"=>$street,"Name"=>$sn,"instructor_name__c"=>$session_Instructor_name,"session_seat_status__c"=>$signupstatus,"session_signup_datetime__c"=>$session_enrolled_date,"ClassroomType__c"=>$classroomtype,"Session_LMS_Id__c"=>$session_lms_id));
    ...
    function sfdc_update_sesslmsid(){
        
        $access_token = $_SESSION['access_token'];
        $instance_url = $_SESSION['instance_url'];

        $query = "SELECT id from Session__c where Session_LMS_Id__c!=''";
        
        $url1 = "$instance_url/services/data/v50.0/query?q=" . urlencode($query);  
        $json_response1 = get_sfdc_curl($query, $url1, $access_token);    

        $response1 = json_decode($json_response1, true);
  
        foreach((array) $response1['records'] as $key1){
            $sessionsfdcid = $key1['Id'];
            $query = "SELECT id,LMS_Session_ID__c from Session_Attendee__c where Session__c='$sessionsfdcid'";
        
            $url2 = "$instance_url/services/data/v50.0/query?q=" . urlencode($query);  
            $json_response2 = get_sfdc_curl($query, $url2, $access_token);    

            $response2 = json_decode($json_response2, true);
            if($response2){
                echo $session_lms_id = $response2['records'][0]['LMS_Session_ID__c'];
                continue;
            }
            else{
                echo "\n else ".$sessionsfdcid." \n";
...
exit;
            $session_lms_id = $response2['records'][0]['LMS_Session_ID__c'];

            $url = "$instance_url/services/data/v50.0/sobjects/Session__c/$sessionsfdcid";

            $content = json_encode(array("Session_LMS_Id__c"=>$session_lms_id));
    ...
    function sfdc_update_session_lms_id($id,$session_lms_id=null,$cancel_time=null){
        $access_token = $_SESSION['access_token'];
        $instance_url = $_SESSION['instance_url'];

        $url = "$instance_url/services/data/v50.0/sobjects/Session__c/$id";

        $content = json_encode(array("Cancellation_Date__c"=>$cancel_time,"Session_LMS_Id__c"=>$session_lms_id));
    ...
    function sfdc_update_classroom_session($id, $classroom_type=null,$duration,$level_type,$location,$manager_email,$session_start_date,$session_end_date,$sn,$street,$session_Instructor,$session_enrolled_date,$signupstatus,$classroomtype=null,$cancel_time=null,$session_lms_id=null){
...
// 	}
        ...
        $content = json_encode(array("classroom_type__c" => $classroom_type,"duration__c"=>$duration,"level_type__c"=>$level_type,"location__c"=> $location,"manager_email__c"=>$manager_email,"session_start_date__c"=>$session_start_date,"session_end_date__c"=>$session_end_date,"street__c"=>$street,"Name"=>$sn,"instructor_name__c"=>$session_Instructor_name,"session_seat_status__c"=>$signupstatus,"session_signup_datetime__c"=>$session_enrolled_date,"ClassroomType__c"=>$classroomtype,"Cancellation_Date__c"=>$cancel_time,"Session_LMS_Id__c"=>$session_lms_id));
//         echo "\n Testid \n";
// //         echo $id;
// //         echo "\n";
// print_r($content);
// // exit;
    ...
    function sfdc_create_users_contact($fname,$lname,$email,$phone,$comp,$address_one,$address_two=NULL,$city,$state,$country,$zip=NULL,$tz=NULL){
        ...
        $url = "$instance_url/services/data/v50.0/sobjects/Contact/";
...
//Delete contact details end - customo
    ...
    //     $url = "$instance_url/services/data/v50.0/sobjects/classroom_cancel_registration__c/";
    ...
    //GET SESSION LMS ID NULL RECORD
    function showSessionLMSIDNULL(){
        $access_token = $_SESSION['access_token'];
        $instance_url = $_SESSION['instance_url'];
        ...
        //echo "<pre>";print_r($response); exit("Succa1");
        foreach ((array) $response['records'] as $record) {
          echo $id=$record['Id']; echo "\n";
           sfdc_delete_session_by_Id($id);
        }
    }

///Delete sessionattendee details by Id - customo
    function sfdc_delete_sessattendee_by_Id($id){
        $access_token = $_SESSION['access_token'];
        $instance_url = $_SESSION['instance_url'];

        $url = "$instance_url/services/data/v50.0/sobjects/Session_Attendee__c/$id";
    ...
    //     $url = "$instance_url/services/data/v50.0/sobjects/classroom_cancel_registration__c/$id";
...
////Task #2524, added new argument $assignId & $studentId.
    function sfdc_create_exam($course_sfdc_id,$cm_lms_id,$module_exam_name,$exam_grade,$exam_design_attempts,$exam_time_modified,$instructor_feedback,$exam_submission_status="Yes",$sfdc_contact_id,$newformat='', $assignId='', $studentId='') {
        ...
        $url = "$instance_url/services/data/v50.0/sobjects/Exam__c/";
...
if(strlen($instructor_feedback) >= 100){
 ...
 // if($studentId == 835029880){ echo "<pre>"; echo "<br>"; echo $instance_url; print_r($_SESSION['access_token']);echo "<br>";
 // print_r($reqData);
 //                                exit("InsertIn");
 //                            }
...
//Delete exam details
    function sfdc_delete_exam($id) {
        ...
        $url = "$instance_url/services/data/v50.0/sobjects/Exam__c/$id";
...
//        $response = json_decode($json_response, true);
  //      $id = $response["id"];
     ...
     function sfdc_delete_cert($id) {
        ...
        $url = "$instance_url/services/data/v50.0/sobjects/Certificate__c/$id";
...
if(strlen($instructor_feedback) >= 100){
    ...
    function create_lesson($course_sfdc_id, $lesson_name,$time_modified,$cm_lms_id,$lesson_completion_status) {
        ...
        $url = "$instance_url/services/data/v50.0/sobjects/Lesson__c/";
...
//echo "\n"; print_r($content); echo "\n";exit("Successss");
    ...
    function create_assessment($course_sfdc_id,$cm_lms_id,$module_quiz_name,$assessment_grade,$assessment_attempts ,$assessment_time_modified) {
        ...
        $url = "$instance_url/services/data/v50.0/sobjects/Assessment__c/";
...
$certificate_sfdc_id = $response['records'][0]['Id'];
		$query2 = "SELECT id, certificate_expiration_date__c, certificate_date__c from certificate_detail__c where certificate_sfdc_id__c='$certificate_sfdc_id'";
        	$url2 = "$instance_url/services/data/v50.0/query?q=" . urlencode($query2);
...
//Certificate Date Code End
   ...
   /* function create_certificate($certificate_name,$certified,$contact_sfdc_id,$course_sfdc_id,$user_email) {
        ...
        $url = "$instance_url/services/data/v50.0/sobjects/Certificate__c/";
   ...
   */
    ...
    //one time function
    function update_certificate_email_by_contact($id,$user_email) {
        $access_token = $_SESSION['access_token'];
        $instance_url = $_SESSION['instance_url'];

        $url = "$instance_url/services/data/v50.0/sobjects/Certificate__c/$id";
        
        $content = json_encode(array("email__c"=>$user_email));
    ...
    //Display certificate detail by contactid and email where cert status is no
    function sfdc_show_certificate_contact_and_email($username, $contact_id) {
        $access_token = $_SESSION['access_token'];
        $instance_url = $_SESSION['instance_url'];

        $query="SELECT id from Certificate__c where email__c = '$username' AND contact_sfdc_id__c='$contact_id' and certified__c='no'";
        $url = "$instance_url/services/data/v50.0/query?q=" . urlencode($query);
        $json_response = get_sfdc_curl($query, $url, $access_token);
...
// /echo "<pre>";print_r($response); exit("Success");
  ...
  //RCP start
    function create_certificate_detail($certificate_date,$certificate_expiration_date,$certificate_sfdc_id,$count_renew_certificate,$expiry,$ren_date){
        ...
        $url = "$instance_url/services/data/v50.0/sobjects/certificate_detail__c/";
    ...
    function update_certificate_detail($id, $certificate_date,$certificate_expiration_date,$certificate_sfdc_id,$count_renew_certificate,$expiry,$ren_date){
        ...
        $url = "$instance_url/services/data/v50.0/sobjects/certificate_detail__c/$id";
        if($certificate_expiration_date == ''){
...
//start - update username field for email and email field for email_2 - QSCID - SSO
        //59Est start
 function sfdc_update_existing_profile($id,$fname,$lname,$username,$email=NULL,$phone,$comp,$address_one,$address_two=NULL,$city,$state,$country,$zip=NULL,$tz=NULL,$region,$sub_region,$usstates=NULL,$rep_firm=NULL,$imp_flag=0,$territory=NULL){
    //59 est end
...
//end - update username field for email and email field for email_2 - QSCID - SSO
 ...
 /* =========== ADD NEW FIELD TO LMS SALESFORCE FEED (Last Access) - END BY PAWAN ================ */
  function update_certificate_details_profile($userid,$pagename = null){
        global $USER,$DB,$CFG;
    ...
    function update_certificate_details($userid,$pagename = null){
        global $USER,$DB,$CFG;
...
// }
        ...
        //echo $coursesModulesBookExistSQLRS->completionstate; exit;
              if(!empty($coursesModulesBookExistSQLRS)){
                ...
                if($nrs1->timeenrolled != 0){
                    $nrs1->timestarted = $nrs1->timeenrolled;    
                }else{
                    if($nrs1->timestarted == 0){
                        $nrs1->timestarted = time();
                        $nrs1->timeenrolled = time();
                    }else{
                        $nrs1->timeenrolled = $nrs1->timestarted;
                    }                
                }
        ...
        }elseif($coursesModulesBookExistSQLRS->completionstate == 2 && $nrs1->timecompleted == NULL && empty($nsqlSCILObj)){
                  //  $coursesModulesQuizExistSQL = "SELECT cmc.id, cmc.completionstate FROM {course_modules} cm JOIN {course_modules_completion} cmc ON cmc.coursemoduleid = cm.id WHERE cm.course =".$keyCertVal->course." and cm.visible=1 and cm.module IN (16) and cmc.userid=".$userid." order by cmc.completionstate asc limit 0,1";
            //  $coursesModulesQuizExistSQLRS = $DB->get_record_sql($coursesModulesQuizExistSQL);
              // if($coursesModulesQuizExistSQLRS->completionstate == 2 && $nrs1->timecompleted == NULL && empty($nsqlSCILObj)){
              //  // $nrs1->timecompleted = time();
              //  }else{
                    $nrs1->timecompleted = NULL;
                //}
            // if($userid == 835002084905){
            // exit("in code");
            // }
        }
...
// if($userid == 835000598){
//     echo "Userid testing please hold on ". $userid . " Certificat". $keyCertVal->id; echo "<BR>";
//             exit("34");
//             }
   ...
   $sqlSCIL = 'SELECT id,userid, courseid, MAX(timecompletion) as timecompletion FROM {simplecertificate_issue_logs} scil where userid IN ('.$userid.') and courseid = '.$keyCertVal->course.' AND timecompletion is not null group by userid, courseid';
          ...
          if($keyCertVal->certexpirydateinyear == 99 || $keyCertVal->course == 42){
            if($sqlSCILObj){
                ...
                // if($userid == 835020580){echo $keyCertVal->course; echo "<br>";}
                $get_assign_idSQL= "select id from mdl_assign where course=".$keyCertVal->course;
                $get_assign_idRes = $DB->get_record_sql($get_assign_idSQL);
                $assignid = $get_assign_idRes->id;
                //if($userid == 835020580 && $keyCertVal->course==42){echo $keyCertVal->course; print_r($get_assign_idRes); echo "<br>check0"; exit;}
                if($assignid){
                    $sqlUserAssignGradeStatus = "SELECT grade, assignment, timemodified FROM `mdl_assign_grades` WHERE `assignment` =$assignid and grade >= 70 AND userid = $userid";                
                    $rsUserAssignGradeStatus = $DB->get_record_sql($sqlUserAssignGradeStatus);
                    if($rsUserAssignGradeStatus){
                      $nsql12 = "SELECT * FROM {course_completions} WHERE userid = $userid AND course = $keyCertVal->course order by timeenrolled";
                      $nrs12 = $DB->get_record_sql($nsql12);
                      if($nrs12){
                       // if($userid == 835020580 && $keyCertVal->course==42){echo $keyCertVal->course; echo "<br>check1"; exit;}
                            $nrs12->timecompleted=$rsUserAssignGradeStatus->timemodified;
                            $DB->update_record('course_completions',$nrs12);
                        }
                        else{
                           // if($userid == 835020580 && $keyCertVal->course==42){echo $keyCertVal->course; echo "<br>check2"; exit;}
                             $update_header_log_assign = "insert into mdl_simplecertificate_issue_logs (userid,timecompletion,courseid) values (".$userid.",".$rsUserAssignGradeStatus->timemodified.",".$keyCertVal->course.")";
                            $DB->execute($update_header_log_assign);
                        }
                    }
                } //end if assignId
        ...
        $coursemoduleArr = "3,16,15";
        $sectionDataSql = "SELECT cs.id, cs.sequence FROM {course_sections} cs JOIN {course_modules} cm ON cs.id = cm.section WHERE cm.course = $keyCertVal->course AND cm.module IN($coursemoduleArr) GROUP BY cm.section";
        $sectionDataRs = $DB->get_records_sql($sectionDataSql);
        ...
        foreach ($sectionDataRs as $key) {
            ...
            $completeCoursesModulesDataSQL = "SELECT cmc.id,cmc.userid,cmc.coursemoduleid FROM {course_modules} cm JOIN {course_modules_completion} cmc ON cmc.coursemoduleid = cm.id WHERE cm.course =$keyCertVal->course and cmc.userid= $userid AND cmc.coursemoduleid IN($sequence) AND cmc.completionstate != 3 and cm.course < 128";
        ...
        elseif($moduleCompleted == count($sectionDataRs)){
            
            $nsql12 = "SELECT * FROM {course_completions} WHERE userid = $userid AND course = $keyCertVal->course order by timeenrolled";
            $nrs12 = $DB->get_record_sql($nsql12);
            if($nrs12){

             if($keyCertVal->course != 7){
                    $get_assign_idSQL= "select id from mdl_assign where course=".$keyCertVal->course;
                    $get_assign_idRes = $DB->get_record_sql($get_assign_idSQL);
                    $assignid = $get_assign_idRes->id;
                }
...
//if($userid == 835059494 && $keyCertVal->course == 26){ echo $assignid; exit;}
                if($keyCertVal->course == 7){
                 $sqlUserAssignGradeStatus = "SELECT grade, assignment, timemodified FROM `mdl_assign_grades` WHERE `assignment` IN (10,18) and grade >= 70 AND userid = $userid";
                }elseif($assignid){
                 $sqlUserAssignGradeStatus = "SELECT grade, assignment, timemodified FROM `mdl_assign_grades` WHERE `assignment` =$assignid and grade >= 70 AND userid = $userid";
                }else{

                  $nrs12->timecompleted=time();
                  $DB->update_record('course_completions',$nrs12);
                }
                
                if($sqlUserAssignGradeStatus)
                    $rsUserAssignGradeStatus = $DB->get_record_sql($sqlUserAssignGradeStatus);
                if($rsUserAssignGradeStatus){
                    $nrs12->timecompleted=$rsUserAssignGradeStatus->timemodified;
                    $DB->update_record('course_completions',$nrs12);

                    $sqlSCI23 = 'SELECT id, userid, certificateid, timecreated, timeexpired
                              FROM {simplecertificate_issues} sci                  
                             WHERE certificateid='.$keyCertVal->id.' and userid='.$userid;
                    $issueCertCompNotiObjData23 = $DB->get_record_sql($sqlSCI23);

                    if(empty($issueCertCompNotiObjData23)){
                        $sqlSCILIntial = 'SELECT * FROM {simplecertificate_issue_logs} scil where  courseid = '.$keyCertVal->course.' and userid='.$userid.' AND timecompletion is not null order by timecompletion asc limit 0,1';
                        $sqlSCILObjIntial = $DB->get_record_sql($sqlSCILIntial);

//if($userid == 835002084123){print_r($sqlSCILObjIntial);exit;}
                        if(!empty($sqlSCILObjIntial)){
                            if($certExpDurationDate != 0){
                                $certExpiredDate = strtotime($certExpDurationDate." years", $sqlSCILObjIntial->timecompletion);
                            }
                            else{
                                $certExpiredDate = strtotime("+1 hour", $sqlSCILObjIntial->timecompletion);
                            }

                            $texp = $certExpiredDate;
                           //if($userid == 835002084123){exit("1");}
                            $insert_header = "INSERT INTO mdl_simplecertificate_issues (userid, certificateid, certificatename, status, timecreated, timeexpired) VALUES (".$userid.", ".$keyCertVal->id.", '".$keyCertVal->name."',0, '".$sqlSCILObjIntial->timecompletion."', '".$texp."')";
                            $DB->execute($insert_header);

                            $insert_header2 = "INSERT INTO mdl_simplecertificate_issue_change_log (userid, courseid, timecreated, timeexpired, sfdc_flag) VALUES (".$userid.",".$keyCertVal->course.",'".$sqlSCILObjIntial->timecompletion."','".$texp."',0)";
                            $DB->execute($insert_header2);

                            $update_header2 = "update mdl_simplecertificate_issue_logs set flag_update_in_sci = 1 where id = $sqlSCILObjIntial->id and courseid = ".$keyCertVal->course;
                                $DB->execute($update_header2);
                        }
                    }
                }
    ...
    }//if($userid == 835059494){exit;}
        return 0;
    }


 function update_certificate_details2($userid){
        global $USER,$DB,$CFG;
        
        $certNotiObjData = $DB->get_records('simplecertificate',null,'','id,name,course,certexpirydateinyear');
        foreach ($certNotiObjData as $keyCertVal) {
if($keyCertVal->course == 97){
            //observation-1 start
            $courseId = $keyCertVal->course;
...
echo "\n";echo "\n";
         echo   $sqlSCIL = 'SELECT id,userid, courseid, MAX(timecompletion) as timecompletion FROM {simplecertificate_issue_logs} scil where userid IN ('.$userid.') and courseid = '.$keyCertVal->course.' AND timecompletion is not null group by userid, courseid';
            $sqlSCILObj = $DB->get_records_sql($sqlSCIL);
echo "\n";
                print_r($sqlSCILObj);
                echo "\n"; //exit;
...
//echo "<br>";
     ...
     function sfdc_update_certdetails_in_cert_onetime($cert_id,$cert_date,$cert_exp_date,$expiry,$ren_status) {
       ...
       $access_token = $_SESSION['access_token'];
        ...
        $url = "$instance_url/services/data/v50.0/sobjects/Certificate__c/$cert_id";
        ...
        if($assignId!='' && $studentId!=''){
            ...
            if(!empty($lastGradeObj)){
               ...
               date_default_timezone_set('GMT');
                $returnDate = date("Y-m-d", $lastGradeObj->timecreated);
                $returnDate .= "T";
                $returnDate .= date("H:i:s", $lastGradeObj->timecreated);
                $returnDate .= "Z";
