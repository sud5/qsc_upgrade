...
function googleoauth2_html_button($authurl, $providerdisplaystyle, $provider) {
        // updated for qsc login link
                
        if($provider->readablename == 'qscid'){
            $provider_readablename = get_string('signinwithanaccountqscid', 'auth_googleoauth2', $provider_readablename);
            return '<a class="singinprovider" href="' . $authurl . '" style="' . $providerdisplaystyle .'">' . $provider_readablename .'</a>';

        } else { 
            $provider_readablename = get_string('signinwithanaccount', 'auth_googleoauth2', $provider_readablename);
            return '<a class="singinprovider" href="' . $authurl . '" style="' . $providerdisplaystyle .'">
                  <div class="social-button ' . $provider->sskstyle . '">' . $provider_readablename .
                 '</div>
                </a>';
        }
 ...
 */
function provider_list() {
    return array('qscid','google', 'facebook', 'battlenet', 'github', 'linkedin', 'messenger', 'microsoft', 'dropbox');
}
/**
...
function googleoauth2_provider_redirect($providername) {
    ...
    // Commented by lakhan to stop checking state
...
function auth_googleoauth2_render_buttons() {
    ...
    if ( count ($providers) > 0 ) {
        ...
        foreach ($providers as $providername) {
            ...
            $providerdisplaystyle = $displayprovider ? 'padding:10px;' : 'display:none;';

            // The button html code.
            $html .= $provider->html_button($authurl, $providerdisplaystyle);
        }
        // updated for qsc login
        if($providername == 'qscid'){
             $html .= '<a href="'. $CFG->wwwroot . (!empty($CFG->alternateloginurl) ? $CFG->alternateloginurl : '/login/index.php')
